...
namespace Triwoinmag
{
    ...
    public class Laser : NetworkBehaviour, IWeapon
    {
        ...
        private float _lineRenAnimSpeed = 10; // Speed for animating UV

        private float _lineRenAnimDeltaTime;

        [Header("Links")] [SerializeField] private LineRenderer _lineRenderer;
        ...
        private void Update()
        {
            ...
            _lineRenderer.material.SetTextureOffset("_MainTex",
                new Vector2(_lineRenAnimDeltaTime * _lineRenAnimSpeed, 0f)); //_BaseMap
        }
        ...
        public void AttackServerRpc(Vector3 targetPosition)
        {
            ...
            if (Physics.Raycast(transform.position, direction, out hitInfo, MaxDistance, _layerMask))
            {
                ...
                {
                    Debug.Log(
                        $"Attack. Physics.Raycast == true. hitInfo.transform.name: {hitInfo.transform.name} hitInfo.point: {hitInfo.point}");
                ...
                if (Debugging)
                    Debug.Log(
                        $"Attack. targetHit != null targetHitName: {targetHit.name} targetHitPos: {targetHit.position}");
        ...
        public void VisualizeFiring(Vector3 targetPosition)
        {
            if (Debugging)
                Debug.Log(
                    $"VisualizeFiring. transform.position: {transform.position}, targetPosition: {targetPosition}");
